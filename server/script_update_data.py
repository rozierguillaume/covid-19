...
def push(type_data):
    ...
    subprocess.run(["sudo", "git", "commit", "-m", f"[auto] data update: {type_data}"])
...
def try_update_france():
    ...
    t1 = dt.datetime.now()
    t2 = datetime_spf
    print(f"diff t1 t2: {abs(t1 - t2)}")
    print(f"abs(t1 - t2).total_seconds()/3600 = {abs(t1 - t2).total_seconds() / 3600}")
    if ( abs(t1 - t2).total_seconds() / 3600 <= 2 ): # Si le fichier SPF date d'il y a moins de 2h
        metadata = requests.get(url_metadata)
        content = str(metadata.content)

        print(f"starting France update: {str(now.hour)}:{str(now.minute)}")
        update_repo()
...
while True:
    ...
    if (now.hour >= 7 and now.hour <= 12 and world_update.day != now.day):
        print(f"starting world update: {str(now.hour)}:{str(now.minute)}")
        # S'il est 05h05
    ...
    if (now.hour == 19 and 21 <= now.minute <= 30):  # (((now.hour == 18) & (now.minute >= 58)) or ((now.hour >= 19) & (now.hour<= 20)))
        print(f"if condition - now: {now}, datetimes_spf: {datetime_spf}")
        while (((now.hour == 18 and now.minute >= 59) or (now.hour >= 19 and now.minute >= 16 and now.hour <= 20)) and (now - datetime_spf).total_seconds()/3600 > 2.5):

            print(f"while loop - now: {now}, datetimes_spf: {datetime_spf}")  # Si heure comprise entre 18h59 et 21h59 ET données PAS à jour depuis plus de 2h30
            now = dt.datetime.now()
